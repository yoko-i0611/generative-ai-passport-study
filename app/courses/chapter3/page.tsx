...
export default function Chapter3Page() {
  ...
  const chapter3 = courses.find(course => course.id === 'chapter3');
  const questions = chapter3?.quiz[0]?.questions || [];
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className={`p-8 rounded-2xl mb-8 ${getColorClass(chapter3.color)}`}>
            <div className={`inline-block px-3 py-1 rounded-full text-sm font-semibold mb-4 ${getBadgeClass(chapter3.color)}`}>
              {chapter3.difficulty}
            </div>
            <h1 className="text-4xl font-bold text-gray-900 mb-2">{chapter3.title}</h1>
            <p className="text-gray-600 text-lg mb-4">{chapter3.description}</p>
            <div className="flex items-center space-x-4 text-sm text-gray-600">
              <div className="flex items-center">
                <Clock className="w-4 h-4 mr-1" />
                <span>{chapter3.time}</span>
              </div>
            </div>
        </div>
        ...
        {!showQuiz ? (
          ...
          <div className="bg-white rounded-lg shadow-sm p-8 border">
            <div className="prose max-w-none">
              <ReactMarkdown>{chapter3.content}</ReactMarkdown>
            </div>
          </div>
        ) : (
          /* 問題演習 */
