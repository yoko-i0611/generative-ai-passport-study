...
### 開発サーバーが起動しない・500エラーが発生する場合

**症状**: `Operation not permitted (os error 1)` エラーが発生
...
**重要な注意事項**:
- macOSのセキュリティ機能やサンドボックの制限により、`node_modules`へのアクセスが制限される場合があります
- 開発サーバーは**ターミナルで直接実行**することを推奨します（IDE内からの起動よりも安定します）
- 詳細は [`開発サーバー起動手順.md`](./開発サーバー起動手順.md) を参照してください

### その他のトラブルシューティング

- **ビルドエラー**: `.next`ディレクトリを削除して再ビルド
- **依存関係のエラー**: `node_modules`を削除して再インストール
- **ポート3000が使用中**: 既存のプロセスを停止してから再起動
